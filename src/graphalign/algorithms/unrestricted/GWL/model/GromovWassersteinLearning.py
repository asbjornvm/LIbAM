...
class GromovWassersteinEmbedding(nn.Module):
    ...
    def self_cost_mat(self, index, idx):
        ...
        if self.cost_type == 'cosine':
            # cosine similarity
            # (batch_size, 1)
            energy = torch.sqrt(torch.sum(embs ** 2, dim=1, keepdim=True))
            cost = 1-torch.exp(-5*(1-torch.matmul(embs, torch.t(embs)) /
                                   (torch.matmul(energy, torch.t(energy)) + 1e-5)))
        else:
            # Euclidean distance
            # (batch_size, batch_size)
            embs = torch.matmul(embs, torch.t(embs))
            # (batch_size, batch_size)
            embs_diag = torch.diag(embs).view(-1, 1).repeat(1, embs.size(0))
            cost = 1 - \
                torch.exp(-(embs_diag + torch.t(embs_diag) -
                            2 * embs)/embs.size(1))
        return cost
    ...
    def mutual_cost_mat(self, index1, index2):
        ...
        if self.cost_type == 'cosine':
            # cosine similarity
            # (batch_size1, 1)
            energy1 = torch.sqrt(torch.sum(embs1 ** 2, dim=1, keepdim=True))
            # (batch_size2, 1)
            energy2 = torch.sqrt(torch.sum(embs2 ** 2, dim=1, keepdim=True))
            cost = 1-torch.exp(-(1-torch.matmul(embs1, torch.t(embs2)) /
                                 (torch.matmul(energy1, torch.t(energy2))+1e-5)))
        else:
            # Euclidean distance
            # (batch_size1, batch_size2)
            embs = torch.matmul(embs1, torch.t(embs2))
            # (batch_size1, batch_size2)
            embs_diag1 = torch.diag(torch.matmul(embs1, torch.t(
                embs1))).view(-1, 1).repeat(1, embs2.size(0))
            # (batch_size2, batch_size1)
            embs_diag2 = torch.diag(torch.matmul(embs2, torch.t(
                embs2))).view(-1, 1).repeat(1, embs1.size(0))
            cost = 1 - \
                torch.exp(-(embs_diag1 + torch.t(embs_diag2) -
                            2 * embs)/embs1.size(1))
        return cost

    def tensor_times_mat(self, cost_s, cost_t, trans, mu_s, mu_t):
        cost_s = cost_s.to(torch.float32)
        cost_t = cost_t.to(torch.float32)
        trans = trans.to(torch.float32)
        mu_s = mu_s.to(torch.float32)
        mu_t = mu_t.to(torch.float32)
        if self.loss_type == 'L2':
            ...
            # cost = cost_st - h1(cost_s)*trans*h2(cost_t)^T
            f1_st = torch.matmul(
                cost_s * torch.log(cost_s + 1e-5) - cost_s, mu_s).repeat(1, trans.size(1))
            f2_st = torch.matmul(torch.t(mu_t), torch.t(
                cost_t)).repeat(trans.size(0), 1)
            cost_st = f1_st + f2_st
            cost = cost_st - \
                torch.matmul(torch.matmul(cost_s, trans),
                             torch.t(torch.log(cost_t + 1e-5)))
        return cost
    ...
    def similarity(self, cost_pred, cost_truth, mask=None):
        if mask is None:
            ...
            if self.loss_type == 'L2':
                ...
                loss = mask.data * ((cost_pred - cost_truth)
                                    ** 2) * torch.exp(-cost_truth)
            else:
                loss = mask.data * \
                    (cost_pred * torch.log(cost_pred / (cost_truth + 1e-5)))
        loss = loss.sum()
        return loss
    ...
    def forward(self, index1, index2, trans, mu_s, mu_t, cost1, cost2, prior=None, mask1=None, mask2=None, mask12=None):
        ...
        regularizer = self.similarity(
            cost_s, cost1, mask1) + self.similarity(cost_t, cost2, mask2)
...
class GromovWassersteinLearning(object):
    ...
    def __init__(self, hyperpara_dict):
        ...
        self.gwl_model = GromovWassersteinEmbedding(
            self.src_num, self.tar_num, self.dim, self.loss_type)
        self.d_gw = []
    ...
    def plot_result(self, index_s, index_t, epoch, prefix):
        ...
        embs = np.concatenate(
            (embs_s.cpu().data.numpy(), embs_t.cpu().data.numpy()), axis=0)
        ...
        plt.savefig('{}/emb_epoch{}_{}_{}.pdf'.format(prefix,
                                                      epoch, self.ot_method, self.cost_type))
        plt.close("all")
        ...
        for i in range(trans_b.shape[0]):
            ...
            trans_b[i, idx] = 1
        plt.imshow(trans_b)
        plt.savefig('{}/trans_epoch{}_{}_{}.png'.format(prefix,
                                                        epoch, self.ot_method, self.cost_type))
        plt.close('all')
    ...
    def regularized_gromov_wasserstein_discrepancy(self, cost_s, cost_t, cost_mutual, mu_s, mu_t, hyperpara_dict):
        ...
        if self.loss_type == 'L2':
            ...
            f2_st = torch.matmul(
                torch.t(mu_t), torch.t(cost_t ** 2)).repeat(ns, 1)
            cost_st = f1_st + f2_st
            for t in range(hyperpara_dict['outer_iteration']):
                cost = cost_st - 2 * \
                    torch.matmul(torch.matmul(cost_s, trans),
                                 torch.t(cost_t)) + 0.1*cost_mutual
            ...
            # cost = cost_st - h1(cost_s)*trans*h2(cost_t)^T
            f1_st = torch.matmul(
                cost_s * torch.log(cost_s + 1e-5) - cost_s, mu_s).repeat(1, nt)
    ...
    def train_without_prior(self, database, optimizer, hyperpara_dict, scheduler=None):
        ...
        """
        device = torch.device(
            'cuda:0' if hyperpara_dict['use_cuda'] else 'cpu')
        if hyperpara_dict['use_cuda']:
            torch.cuda.manual_seed(1)
        kwargs = {'num_workers': 1,
                  'pin_memory': True} if hyperpara_dict['use_cuda'] else {}
            ...
            for src_idx, indices1 in enumerate(src_loader):
                for tar_idx, indices2 in enumerate(tar_loader):
                    ...
                    if hyperpara_dict['display']:
                        self.plot_result(index_s, index_t, epoch,
                                         prefix=hyperpara_dict['prefix'])
                    ...
                    if hyperpara_dict['strategy'] == 'hard':
                        ...
                        if z < epoch/hyperpara_dict['epochs']:
                            ...
                            cost1 = self.gwl_model.self_cost_mat(
                                index_s, 0).data
                            cost2 = self.gwl_model.self_cost_mat(
                                index_t, 1).data
                            cost12 = self.gwl_model.mutual_cost_mat(
                                index_s, index_t).data
                        else:
                            cost1 = cost_s
                            cost2 = cost_t
                            cost12 = 0
                        ...
                        cost_s_emb = self.gwl_model.self_cost_mat(
                            index_s, 0).data
                        cost_t_emb = self.gwl_model.self_cost_mat(
                            index_t, 1).data
                        cost_st_12 = self.gwl_model.mutual_cost_mat(
                            index_s, index_t).data
                        # alpha = max([(hyperpara_dict['epochs'] - epoch) / hyperpara_dict['epochs'], 0.5])
                        alpha = (hyperpara_dict['epochs'] -
                                 epoch) / hyperpara_dict['epochs']
                    ...
                    for row in range(trans_np.shape[0]):
                        for col in range(trans_np.shape[1]):
                            trans_tmp[index_s_np[row], index_t_np[col]
                                      ] += (energy * trans_np[row, col])
    ...
    def train_with_prior(self, database, optimizer, hyperpara_dict, scheduler=None):
        ...
        train_base['mutual_interactions'] = train_base['mutual_interactions'][:int(
            0.75*num_interaction)]
        test_base['mutual_interactions'] = test_base['mutual_interactions'][int(
            0.75*num_interaction):]
        ...
        for epoch in range(hyperpara_dict['epochs']):
            ...
            for batch_idx, indices in enumerate(dataloader):
                ...
                if hyperpara_dict['strategy'] == 'hard':
                    ...
                    if z < epoch / hyperpara_dict['epochs']:
                        cost1 = mask_s.data * \
                            self.gwl_model.self_cost_mat(index_s, 0).data
                        cost2 = mask_t.data * \
                            self.gwl_model.self_cost_mat(index_t, 1).data
                        cost12 = mask_st.data * \
                            self.gwl_model.mutual_cost_mat(
                                index_s, index_t).data
                    else:
                        cost1 = cost_s
                        cost2 = cost_t
                    ...
                    cost_s_emb = mask_s.data * \
                        self.gwl_model.self_cost_mat(index_s, 0).data
                    cost_t_emb = mask_t.data * \
                        self.gwl_model.self_cost_mat(index_t, 1).data
                    cost_st_12 = mask_st.data * \
                        self.gwl_model.mutual_cost_mat(index_s, index_t).data
                    alpha = max(
                        [(hyperpara_dict['epochs'] - epoch) / hyperpara_dict['epochs'], 0.7])
                    cost1 = alpha * cost_s + (1 - alpha) * cost_s_emb
                    cost2 = alpha * cost_t + (1 - alpha) * cost_t_emb
                ...
                logger.info('Train Epoch: {}'.format(epoch))
                logger.info(
                    '- OT method={}, Distance={}'.format(self.ot_method, self.cost_type))
                tops = [1, 3, 5]
                for top in range(3):
    ...
    def save_model(self, full_path, mode: str = 'entire'):
        ...
        if mode == 'entire':
            ...
            logger.info(
                'The parameters of the model is saved in {}.'.format(full_path))
        else:
            logger.warning(
                "'{}' is a undefined mode, we use 'entire' mode instead.".format(mode))
            torch.save(self.gwl_model, full_path)
            logger.info('The entire model is saved in {}.'.format(full_path))
