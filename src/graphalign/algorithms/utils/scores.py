...
def calc_frac_idx(x1_mat, x2_mat):
    ...
    for row_idx in range(nsamp):
        euc_dist = np.sqrt(
            np.sum(np.square(np.subtract(x1_mat[row_idx, :], x2_mat)), axis=1)
        )
...
def transfer_accuracy_svm(y1, y2, Z1, Z2, C=1.0):
    from sklearn.svm import LinearSVC

    clf = LinearSVC(C=C)
    clf.fit(Z1, y1)
    y2_pred = clf.predict(Z2)
    acc = np.sum(y2_pred == y2) / len(y2)
    return acc
