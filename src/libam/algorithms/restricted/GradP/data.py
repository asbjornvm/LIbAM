...
def Reordering(G1, G2):
    ...
    G2 = nx.convert_node_labels_to_integers(G2, first_label=G1.number_of_nodes() + 1, ordering='default',
...
class Dataset:
    ...
    def _load_G(self):
        ...
        G_data['links'] = [
            {'source': self.idx2id[G_data['links'][i]['source']], 'target': self.idx2id[G_data['links'][i]['target']]}
            for i in range(len(G_data['links']))]
...
def augment_attributes(Gs, Gt, attr_s, attr_t, num_attr, version="katz", khop=1, penalty=0.1, normalize=True):
    Gs_nodes = list(Gs.nodes())
    Gt_nodes = list(Gt.nodes())
    logging.getLogger(__name__).debug(f"This is {version} binning version ")
    if version == "katz":
        ...
        attdict_s_2hop = reduce(lambda d1, d2: {k: d1.get(k, 0) + d2.get(k, 0) for k in set(d1) | set(d2)},
                                dict_seq_2_s)
        attdict_t_2hop = reduce(lambda d1, d2: {k: d1.get(k, 0) + d2.get(k, 0) for k in set(d1) | set(d2)},
                                dict_seq_2_t)
        ...
        attdict_s_3hop = reduce(lambda d1, d2: {k: d1.get(k, 0) + d2.get(k, 0) for k in set(d1) | set(d2)},
                                dict_seq_3_s)
        attdict_t_3hop = reduce(lambda d1, d2: {k: d1.get(k, 0) + d2.get(k, 0) for k in set(d1) | set(d2)},
                                dict_seq_3_t)
