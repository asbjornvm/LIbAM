import torch
import numpy as np
from libam.algorithms.restricted.HTC.utils import *
from torch import optim
import torch.nn.functional as F
from dataclasses import dataclass

from libam._logging import logger
from libam.algorithms.restricted.HTC.utils import CSLS
from libam.graph.graph_pair import GraphPair
from libam.algorithms.restricted.HTC.MyNet import MyNet, Reconstruction_loss
from libam.algorithms.algorithm import AlignAlgorithm


@dataclass
class HTC(AlignAlgorithm):
    pair: GraphPair
    # Model
    hid_dim: int
    ...
    k: int
    p: float
    # Graphlet-orbit Laplacians; built from the pair when not provided.
    src_laps: torch.Tensor | None = None
    trg_laps: torch.Tensor | None = None
    graphlet_size: int = 4

    def __post_init__(self) -> None:
        self.__name__: str = "HTC"
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.src_feat = build_features(
            self.pair.src_features, self.pair.src.number_of_nodes()).to(self.device)
        self.trg_feat = build_features(
            self.pair.tar_features, self.pair.tar.number_of_nodes()).to(self.device)
        if self.src_laps is None:
            self.src_laps = build_laps(self.pair.src, self.graphlet_size)
        if self.trg_laps is None:
            self.trg_laps = build_laps(self.pair.tar, self.graphlet_size)
    ...
    def unsupervised_train(self, myNet):
        ...
        for epoch in range(self.num_utrn):
            for i in range(len(self.src_laps)):
                ...
                logger.debug('epoch %d | loss_%d: %.4f' % (epoch, i, loss_st))
        ...
        for i in range(len(self.src_laps)):
            ...
            logger.debug('epoch %d | loss_%d: %.4f' % (epoch + 1, i, loss_st))
        return myNet
        ...
        for epoch in range(self.num_ftune):
            ...
            for i in range(len(self.src_laps)):
                if tune_flag[i] == False:
                    logger.debug('epoch %d_%d: undo' % (epoch, i))
                    continue
                src_output = myNet(self.src_laps[i], self.src_feat)
                trg_output = myNet(self.trg_laps[i], self.trg_feat)
                ...
                if count > count_max[i]:
                    ...
                    tune_flag[i] = False
                logger.debug('epoch %d_%d: mutual closest pairs: %d' % (epoch, i, count))
        return myNet, count_max
