...
def build_laps(G: nx.Graph, graphlet_size: int = 4) -> torch.Tensor:
    """Build the stack of normalized graphlet-orbit Laplacians for one graph"""
    G = G.copy()
    G.remove_edges_from(nx.selfloop_edges(G))
    adj = nx.adjacency_matrix(G)
    orbit_counts = orbit_count.edge_orbit_counts(G, graphlet_size=graphlet_size)
    edges = np.array(list(G.edges()))
    goms = orca2gom(edges, orbit_counts, adj)
    return torch.Tensor(gom2lap(goms))

def build_features(feats: np.ndarray | None, num_nodes: int) -> torch.Tensor:
    """Normalize node features, falling back to a structural one-hot"""
    if feats is None:
        feats = np.zeros((num_nodes, 1))
    feats = np.array(feats, dtype=float)
    for i in range(len(feats)):
        if feats[i].sum() == 0:
            feats[i, -1] = 1
    return F.normalize(torch.Tensor(feats))
