...
from libam.algorithms.utils.matrix_utils import doubly_stochastic, regularise_and_invert, regularise_invert_one, \
    to_torch, doubly_stochastic_same_type
...
class Got(AlignAlgorithm, ABC):
    ...
    def _align(self) -> np.ndarray | torch.Tensor | scipy.sparse.csr_matrix:
        ...
        [x_reg, y_reg] = regularise_and_invert(A, B, self.alpha, self.ones)
        x = to_torch(x_reg)
        y = to_torch(y_reg)
        n = x.shape[0]
        ...
        # Optimization
        optimizer = torch.optim.Adam([mean,std], lr=self.lr, amsgrad=True)
        history = []
        for epoch in range(self.epochs):
            cost = 0
            cost_vec = np.zeros((1,self.n_samples))
            for sample in range(self.n_samples):
                ...
                # Cost function
                DS = doubly_stochastic_same_type(P_noisy, self.it)
                cost = cost + loss(DS, x, y, params, self.loss_type)
                cost_vec[0,sample] = loss(DS, x, y, params, self.loss_type)
            ...
            if self.verbose and (epoch==0 or (epoch+1) % 100 == 0):
                print('[Epoch %4d/%d] loss: %f - std: %f' % (epoch+1, self.epochs, cost.item(), std.detach().mean()))

        # PyTorch -> NumPy
        P = doubly_stochastic(mean, self.it)
