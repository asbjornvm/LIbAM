...
class Grasp(AlignAlgorithm):
    ...
    def _align(self) -> np.ndarray | torch.Tensor | scipy.sparse.csr_matrix:
        ...
        n_eig = self.n_eig if self.n_eig is not None else Src.shape[0] - 1
        G1_emb, G2_emb = functional_maps_gen(
            Src, Tar, q=self.q, k=self.k, n_eig=n_eig, laa=self.laa,
            icp=self.icp, icp_its=self.icp_its, lower_t=self.lower_t,
