...
def netmf_mat_full(A, window=10, b=1.0):
    ...
    for i in range(window):
        logging.getLogger(__name__).debug(f"@{i+1}/{window}")
    ...
    m = torch.tensor(M.todense(), dtype=torch.float32)
    Y = torch.log(torch.clamp(m, min=1)).numpy()
...
def netmf_mat_full1(A, window=10, b=1.0):
    if not sparse.issparse(A):
        A = sparse.csr_matrix(A)
    # print "A shape", A.shape
    n = A.shape[0]#
