...
def align(X, Y, R, lr=1.0, bsz=10, nepoch=5, niter=10,
    ...
    for epoch in range(1, nepoch + 1):
        for _it in range(1, niter + 1):
            ...
            update = lr / bsz * G
            logging.getLogger(__name__).debug(("Update: %.3f (norm G %.3f)" % (np.linalg.norm(update), np.linalg.norm(G))))
            R -= update
        ...
        niter //= 4
        if verbose:
            logging.getLogger(__name__).debug(("epoch: %d  obj: %.3f" % (epoch, objective(X, Y, R))))
