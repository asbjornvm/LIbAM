...
def plot(graph1: nx.Graph, graph2: nx.Graph) -> None:
...
def feature_extractionBM(G: nx.Graph, simple: bool) -> np.ndarray:
...
def Degree_Features(G: nx.Graph, EFN: int) -> np.ndarray:
    CS = []
    BS = []
    DC = []
...
def eucledian_dist(F1: np.ndarray, F2: np.ndarray, n: int) -> np.ndarray:
    D = euclidean_distances(F1, F2)
    return D

def dist(A: np.ndarray, B: np.ndarray, P: np.ndarray) -> float:
...
def convex_initTun(A, B, D, mu, niter):
    ...
    return P
'''
def convex_initTun(A: torch.Tensor, B: torch.Tensor, D: torch.Tensor, K: torch.Tensor, mu: float, niter: int) -> torch.Tensor:
    np.set_printoptions(suppress=True)
    n = len(A)
    P = torch.ones((n,n), dtype = torch.float64)
    P=P/n
    ...
    for i in range(0,11):
        ...
        #P = P + alpha * (q - P)
    return P
def convex_init(A: torch.Tensor, B: torch.Tensor, D: torch.Tensor, mu: float, niter: int) -> torch.Tensor:
    np.set_printoptions(suppress=True)
    device = A.device
    n = len(A)
    P = torch.ones((n,n), dtype = torch.float64, device=device)
    P=P/n
    ones = torch.ones(n, dtype = torch.float64, device=device)
    mat_ones = torch.ones((n, n), dtype = torch.float64, device=device)
    reg = 1.0
    K=mu*D
    ...
    for i in range(niter):
        for it in range(1, 11):
            ...
            P = P + alpha * (q - P)
    return P
def convex_initQAP(A: torch.Tensor, B: torch.Tensor, niter: int) -> torch.Tensor:
    n = len(A)
    P = torch.ones((n,n), dtype = torch.float64)
    P=P/n
    ones = torch.ones(n, dtype = torch.float64)
...
def convertToPermHungarian(M: np.ndarray, n1: int, n2: int) -> tuple[np.ndarray, list[tuple[int, int]]]:
...
def convertToPermGreedy(M: np.ndarray, n1: int, n2: int) -> tuple[np.ndarray, list[tuple[int, int]]]:
    n = len(M)
...
def convertToPerm(A: np.ndarray, B: np.ndarray, M: np.ndarray, n1: int, n2: int) -> tuple[np.ndarray, list[tuple[int, int]]]:
