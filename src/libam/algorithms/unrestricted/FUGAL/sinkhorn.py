...
def sinkhorn(a: torch.Tensor, b: torch.Tensor, C: torch.Tensor, reg: float = 1e-1, method: str = 'sinkhorn', maxIter: int = 1000, tau: float = 1e3,
             stopThr: float = 1e-9, verbose: bool = False, log: bool = False, warm_start: dict | None = None, eval_freq: int = 10, print_freq: int = 200, **kwargs: object) -> torch.Tensor:
    r"""
...
def sinkhorn_knopp(a: torch.Tensor, b: torch.Tensor, C: torch.Tensor, reg: float = 1e-1, maxIter: int = 1000, stopThr: float = 1e-9,
                   verbose: bool = False, log: bool = False, warm_start: dict | None = None, eval_freq: int = 10, print_freq: int = 200, **kwargs: object) -> torch.Tensor:
    r"""
    Solve the entropic regularization optimal transport
    The input should be PyTorch tensors
    The function solves the following optimization problem:
    ...
    # we don't allocate memory beforehand as torch no longer views this as safe
...
def sinkhorn_stabilized(a: torch.Tensor, b: torch.Tensor, C: torch.Tensor, reg: float = 1e-1, maxIter: int = 1000, tau: float = 1e3, stopThr: float = 1e-9,
                        verbose: bool = False, log: bool = False, warm_start: dict | None = None, eval_freq: int = 10, print_freq: int = 200, **kwargs: object) -> torch.Tensor:
    r"""
    ...
    def update_K(alpha: torch.Tensor, beta: torch.Tensor) -> None:
    ...
    def update_P(alpha: torch.Tensor, beta: torch.Tensor, u: torch.Tensor, v: torch.Tensor, ab_updated: bool = False) -> None:
...
def sinkhorn_epsilon_scaling(a: torch.Tensor, b: torch.Tensor, C: torch.Tensor, reg: float = 1e-1, maxIter: int = 100, maxInnerIter: int = 100, tau: float = 1e3, scaling_base: float = 0.75,
                             scaling_coef: float | None = None, stopThr: float = 1e-9, verbose: bool = False, log: bool = False, warm_start: dict | None = None, eval_freq: int = 10,
                             print_freq: int = 200, **kwargs: object) -> torch.Tensor:
    r"""
    Solve the entropic regularization OT problem with log stabilization
    The function solves the following optimization problem:
