...
from libam.graph._generate import apply_noise, permute_graph
from libam.graph._similarities_preprocess import create_L, create_S
...
class GraphPair:
    ...
    [n, d]. When provided, restricted algorithms can use these structural features
    when they are provide. The caller is responsible for ensuring feature rows
    remain consistent with node indices after any permutation or re-labeling.
    """
    src: nx.Graph
    tar: nx.Graph
    ...
    def from_graph(
        ...
        :param G: A single graph of type NetworkX.Graph to derive graph pair from
        :param features: Features accompanying the graphs nodes.
        :return:
        """
        n = G.number_of_nodes()
